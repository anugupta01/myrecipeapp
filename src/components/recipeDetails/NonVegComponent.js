...
import { fetchNonVegRecipes } from '../service/recipeService';
import "./Vegan.css";
import { useNavigate } from "react-router-dom";
import Button from "../CommonUI/Button";

const NonVegComponent = () => {
 ...
 useEffect(() => {
     const getRecipes = async () => {
       try {
         const recipeData = await fetchNonVegRecipes();
  ...
  return (
    <div className="vegan-recipes">
      ...
      <Button className="btn btn-back" onClickCapture={back}>
        Back
      </Button>
      ...
      <div className="recipe-list">
        {filteredRecipes.length > 0 ? (
          ...
          <p>No matching non-vegetarian recipes found.</p>
        )}
      </div>
    </div>
  );
